...
@section('title', 'Transaksi Tiket Harian')
@section('page_title', 'Transaksi Tiket Harian')

@section('content')
  <div class="space-y-6">
    <div>
      <p class="text-xs uppercase tracking-[0.2em] text-slate-400">Transaksi</p>
      <h2 class="text-2xl font-semibold text-slate-900">Tiket Harian</h2>
    </div>
        ...
        <tbody class="divide-y divide-slate-200">
          @forelse ($items as $item)
            <tr class="text-slate-600">
              ...
              <td class="px-4 py-3">
                <div class="font-semibold text-slate-900">{{ $item->transaction?->customer_name ?? '-' }}</div>
                <div class="text-xs text-slate-400">{{ $item->transaction?->customer_email ?? '-' }}</div>
              </td>
              ...
              <td class="px-4 py-3 text-right font-semibold text-slate-800">
                Rp {{ number_format($item->transaction?->amount_total ?? 0, 0, ',', '.') }}
              </td>
            </tr>
          @empty
            <tr>
              <td colspan="8" class="px-4 py-8 text-center text-sm text-slate-500">Belum ada transaksi tiket harian.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
